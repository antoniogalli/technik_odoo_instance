...
class GenerateAndSendPayslip(models.TransientModel):
    _name = 'hr.payroll.generate.send.wizard'
    _description = 'Generate & Send reports hr.payslip'
    ...
    def confirm(self):
        ...
        for payslip in self._context.get('active_ids'):
            ...
            doc_payslip = obj_payslip.action_report_mass(self.report_id.id)
            object_name = str(doc_payslip['attname'])
            if os.path.isfile(object_name):
                os.remove(object_name)
            object_handle = open(object_name, "wb")
            # writing binary data into file handle
            object_handle.write(base64.b64decode(doc_payslip['b64_pdf']))
            object_handle.close()
