    ...
    def render_cpe_page(self, **kw):
        ...
        if http.request.httprequest.method == 'POST':
            try:
                ...
                doc_type = (not req.get('doc_type') or req.get(
                    'doc_type') == "-") and False or req.get('doc_type')
                doc_number = (not req.get('doc_number') or req.get(
                    'doc_number') == "-") and False or req.get('doc_number') or False
                ...
                querry = [('pe_invoice_code', '=', req.get('document_type')), ('partner_id.doc_type', '=', doc_type),
                          ('partner_id.doc_number', '=', doc_number), ('date_invoice',
                                                                       '=', req.get('date_invoice')),
                          ('move_name', '=', "%s-%s" %
                           (num[0], number)), ('amount_total', '=', req.get('amount_total')),
