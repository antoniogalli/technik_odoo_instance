...
class PurchaseOrder(models.Model):
    ...
    def _prepare_invoice(self):
        ...
        if invoice_vals:
            invoice_vals.update(
                {
                    "report_template_id": self.partner_id
                    and self.partner_id.report_template_id.id
                    or False
                }
            )
        return invoice_vals
...
class SaleOrder(models.Model):
    ...
    def _prepare_invoice(self):
        ...
        if invoice_vals:
            invoice_vals.update(
                {
                    "report_template_id": self.partner_invoice_id
                    and self.partner_invoice_id.report_template_id.id
                    or False
                }
            )
        return invoice_vals
...
class AccountMove(models.Model):
    ...
    sunat_state = fields.Selection(related="pe_cpe_id.state", string="Estado SUNAT")
    ...
    def order_lines_layouted(self):
        ...
        if result["group_sale_layout"] != False:
            for category, lines in groupby(
                self.invoice_line_ids, lambda l: l.layout_category_id
            ):
    ...
    def _get_origin_reference(self, origin):
        self.ensure_one()
        if self.type in ("in_invoice", "in_refund"):
            sale_obj = self.env["purchase.order"]
        else:
    ...
    def _get_invoice_date(self):
        ...
        lang = self._context.get("lang")
        lang_obj = self.env["res.lang"]
        ids = lang_obj.search([("code", "=", lang or "es_PE")])
            ...
            try:
                if self.partner_id.lang:
                    self.amount_to_text = num2words(
                        self.amount_total, lang=self.partner_id.lang
                    ).title()
                else:
                    self.amount_to_text = num2words(
