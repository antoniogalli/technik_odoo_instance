    ...
    def send_to_mailchimp(self):
        ...
        try:
            ...
            if not list_audience:
                list = self.env['mailing.list'].export_list_mailchimp()
            ...
            if temp == 1:
                mail_list = self.env['mailing.list'].update_list_mailchimp(list_audience,
                                                                                self.contact_list_ids.ids)
            ...
            if campaign:
                ...
                if temp == 0:
                    history = self.env['mailchimp.lists'].sudo().create(
                        {'mailing_lists': [(6, 0, self.contact_list_ids.ids)],
