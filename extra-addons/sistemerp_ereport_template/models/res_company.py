...
class ResCompany(models.Model):
        ...
        report_id = report_obj.search(
            [
                ...
                (
                    "report_name",
                    "=",
                    "sistemerp_ereport_template.report_invoice_template_custom",
                ),
            ]
        )
        if report_id:
    ...
    def _get_default_image(self, is_company, colorize=False):
        img_path = get_module_resource(
            "sistemerp_ereport_template", "static/src/img", "avatar.png"
        )
    ...
    def _onchange_invoice_template(self):
        if self.standard_template:
            ...
            self.theme_text_color = template_value.get("theme_text_color", "#FFFFFF")
    ...
    def _get_font(self):
        return self.env["report.fonts"].search(
    ...
    theme_color = fields.Char(
        string="Template Base Color",
        required=True,
        help="Please set Hex color for Template.",
        default="#a24689",
    )
    theme_text_color = fields.Char(
        string="Template Text Color",
        required=True,
        help="Please set Hex color for Template Text.",
        default="#FFFFFF",
    )
    text_color = fields.Char(
        string="General Text Color",
        required=True,
        help="Please set Hex color for General Text.",
        default="#000000",
    )
    company_color = fields.Char(
        string="Company Name Color",
        required=True,
        help="Please set Hex color for Company Name.",
        default="#6F8192",
    )
    customer_color = fields.Char(
        string="Customer Name Color",
        required=True,
        help="Please set Hex color for Customer Name.",
        default="#000000",
    )
    company_address_color = fields.Char(
        string="Company Address Color",
        required=True,
        help="Please set Hex color for Company Address.",
        default="#6F8192",
    )
    customer_address_color = fields.Char(
        string="Customer Address Color",
        required=True,
        help="Please set Hex color for Customer Address.",
        default="#000000",
    )
    odd_party_color = fields.Char(
        string="Table Odd Parity Color",
        required=True,
        help="Please set Hex color for Table Odd Parity.",
        default="#FFFFFF",
    )
    even_party_color = fields.Char(
        string="Table Even Parity Color",
        required=True,
        help="Please set Hex color for Table Even Parity.",
        default="#e6e8ed",
    )
    report_template_id1 = fields.Many2one(
        "ir.actions.report",
        string="Invoice Template1",
    ...
    is_description = fields.Boolean(
        string="Display Product Description",
        default=True,
        help="Please check it if you want to show product description in report.",
    )
    watermark_logo = fields.Binary(
        "Report Watermark Logo",
        default=lambda self: self._get_default_image(False, True),
        help="Please set Watermark Logo for Report.",
    )
    is_company_bold = fields.Boolean(
        string="Display Company Name in Bold",
        default=True,
        help="Please check it if you want to show Company Name in Bold.",
    )
    is_customer_bold = fields.Boolean(
        string="Display Customer Name in Bold",
        default=False,
        help="Please check it if you want to show Customer Name in Bold.",
    )
    is_show_invoice_notes = fields.Boolean(
        string="Display Invoice Notes",
        default=False,
        help="Please check it if you want to show Invoice Notes in pdf.",
    )
    is_show_payment_notes = fields.Boolean(
        string="Display Payment Term",
        default=False,
        help="Please check it if you want to show Payment Term in pdf.",
    )
    is_show_payment_description = fields.Boolean(
        string="Display Payment Description",
        default=False,
        help="Please check it if you want to show Payment Description in pdf.",
    )
    standard_template = fields.Selection(
        standard_template,
        string="Standard Template Configuration",
        required=True,
        default="custom",
        help="Please select your choice Standard Color Configuration for all Template.",
    )
    ...
    watermark = fields.Selection(
        selection=[("logo", "Logo"), ("text", "Text"), ("status", "Status")],
        required=True,
    ...
    watermark_text = fields.Char(
        ...
        default="Documento sin valor Legal.",
        help="Please enter watermark text",
    )
    watermark_text_font_size = fields.Integer(
        "Watermark Text Font Size(em)",
        default=6,
        help="Please set watermark text font size",
    )
    watermark_text_color = fields.Char(
        string="Watermark Text Color",
        required=[("watermark", "!=", "logo")],
        help="Please set Hex color for Watermark text.",
        default="#a24689",
    )
    add_product_image = fields.Boolean(
        string="Display Product Image",
        default=False,
        help="Please check it if you want to show Product Image in pdf.",
    )

    font_id = fields.Many2one(
        comodel_name="report.fonts",
        string="Report Font",
    ...
    font_size = fields.Integer(
        ...
        default=12,
        required=True,
        help="Please define report font size",
    )

    is_show_signature = fields.Boolean(
        string="Display Signature",
        default=False,
        help="Please check it if you want to show signature in PDF.",
    )
    signature = fields.Binary(
        string="Signature",
        default=lambda self: self._get_default_image(False, True),
        help="Please upload signature image to display in report.",
    )
    report_footer_selection = fields.Selection(
        selection=[("standard", "Standard"), ("multi_columns", "Multi Columns Footer")],
        default="standard",
        string="Display Report Footer",
        help="Select footer style if you want to show in report",
    )
    logo_footer = fields.Binary(
        "Report Footer Logo",
        compute="_compute_logo_footer",
        store=True,
        help="Please set Footer Logo for Report.",
    )
    is_show_bank_details = fields.Boolean(
        string="Display Bank Details",
        default=False,
        help="Please check it if you want to show Bank details in pdf.",
    )
    report_bank_id = fields.Many2one(
        "res.partner.bank",
        string="Banks",
        help="Please select bank account which you want to show Bank details in pdf.",
    )
    is_show_barcode = fields.Boolean(
        string="Display Report Barcode",
        default=True,
        help="Please check it if you want to show report barcode in pdf.",
    )
    add_amount_in_words = fields.Boolean(
        string="Display Amount In Word",
        default=True,
        help="Please unchecked if you want to hide amount in words",
    )
    ...
    def action_view_report_extra_content(self):
        action = self.env.ref(
            "sistemerp_ereport_template.action_report_extracontent"
        ).read()[0]
    ...
    def check_font_size(self):
        if self.watermark_text_font_size <= 0 or self.watermark_text_font_size > 10:
            raise ValidationError(
                _(
                    "Please input watermark text font size is greater than 0 and lower than 10, otherwise your report will very massive."
                )
            )
        if self.font_size <= 10 or self.font_size >= 25:
            raise ValidationError(
                _(
                    "Please input font size is greater than 10 and lower than 25, otherwise your report will very massive."
                )
            )
