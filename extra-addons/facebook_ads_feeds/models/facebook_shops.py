...
class FacebookMerchantShop(models.Model):
    ...
    @api.onchange('id','shop_url','enable_feed','feed_token')
    ...
    def _get_one_product_mapping(self,product,field_mapping_lines):
        ...
        if self.multi_images:
            ...
            for image in product.product_image_ids:
                additiona_images.append(d.get('BASE_URL')+"/web/image/product.image/"+str(image.id)+"/image/600x600" +"?mlsec=%s"%(int(time() * 1000)))
    ...
    def _get_product_mapping(self,products,field_mapping_lines):
        ...
        for product in products:
            if self.sync_product_variant and product.product_variant_count > 0:
    ...
    def _default_designed_function(self,key,value,d):
        if(key == 'price'):
            ...
            image_url = d.get('BASE_URL')+"/web/image/product.product/"+d.get('ID')+"/image/600x600" +"?mlsec=%s"%(int(time() * 1000))
            return image_url
        elif (key == 'inventory'):
