    ...
    def create(self, vals):
        if vals.get('contract_id'):
            ...
            if contract and str(contract.date_end) == str(vals.get('date_to')):
                contract_completion_id = self.env['hr.contract.completion'].search([('employee_id', '=', vals.get('employee_id')),
                                                                                    ('date', '=', contract.date_end)])
                if not contract_completion_id:
                    raise ValidationError(_("You don't can create this payslip why date end contract is contain in payslip, "
